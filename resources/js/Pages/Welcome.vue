<script setup>
    ...
    onMounted(() => {
        // Automatically fetch latest weather and map data when user enters the site
        if (window.axios) {
            window.axios
                .post(route('weather.refresh_all'))
                .then((res) =>
                    console.log(
                        'Welcome: Auto-fetch complete:',
                        res.data.message,
                    ),
                )
                .catch((err) =>
                    console.error('Welcome: Auto-fetch failed:', err),
                )
        }
    })
</script>
...
<template>
    ...
    <div
        class="relative min-h-screen overflow-hidden font-sans text-white bg-gradient-to-b from-blue-600 to-blue-900"
    >
        ...
        >
            ...
            <p
                class="max-w-2xl mb-12 text-lg font-light leading-relaxed text-blue-100 sm:text-2xl animate-fade-in-up delay-100"
            >
                Seamlessly plan your sea voyages.
                <span class="font-bold text-white">FerryCast</span> delivers
                real-time schedules, weather forecasts, and smart route insights
                for the modern traveler.
            </p>

            <!-- Buttons -->
            <div
                class="flex flex-col w-full max-w-2xl mx-auto space-y-6 animate-fade-in-up delay-200"
            >
                <!-- Main Action Row -->
                <div
                    v-if="canLogin"
                ...
                <!-- Public Access Links -->
                <div
                    class="flex flex-row justify-center items-center gap-6 pt-4 border-t border-white/10"
                >
                    <Link
                        href="/schedules"
                        class="text-blue-200 hover:text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2 transition-colors"
                    >
                        <svg
                            class="w-5 h-5"
                            fill="none"
            ...
            <footer
                class="absolute bottom-4 text-xs text-blue-200/50 animate-fade-in"
            >
